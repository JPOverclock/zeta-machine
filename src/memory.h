...
namespace zm {
    ...
    public:
        ...
        bool read_bit(uint32_t address, uint8_t position) { return false; }
        void write_bit(uint32_t address, uint8_t position) { }

        void write(uint32_t address, uint8_t value) { contents[address] = value; }
        void write_word(uint64_t address, uint16_t value) { contents[address] = value >> 4; contents[address + 1] = (value & 0x00FF); }
